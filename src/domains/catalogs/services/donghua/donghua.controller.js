...
const { paginationQuery, searchQuery, byGenreQuery, byYearQuery } = require('./donghua.validation');
...
const search = catchAsync(async (req, res) => {
  const { q, page, limit } = searchQuery.parse(req.query);
...
const byGenre = catchAsync(async (req, res) => {
  ...
  const { page, limit } = byGenreQuery.parse(req.query);
...
const byYear = catchAsync(async (req, res) => {
  ...
  const { page, limit } = byYearQuery.parse(req.query);
